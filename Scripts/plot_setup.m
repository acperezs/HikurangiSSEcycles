...
% Slip budget over 100 years
slipbudget = load('./slipbudget.txt');
...
xlabel('longitude ( ˚ )');
ylabel('latitude ( ˚ )');
title('Cumulative slip from 55 to 75 years');
...
box on;
set(gca,'xlim',[175 180],'ylim',[-42 -37],'fontsize',12);
scatter(x(:,1),y(:,1),10,slipbudget(:),'filled'); 
colormap('parula');
plot(xcoast,ycoast, 'k', 'LineWidth', 1.5);
xlabel('longitude ( ˚ )');
ylabel('latitude ( ˚ )');
title('Slip budget over 100 years')

ax = gca; ax.FontSize = 15;
cl = colorbar ;
cl.Label.String = '% plate convergence' ;
